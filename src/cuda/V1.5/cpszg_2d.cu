...
unsigned char *
sz_compress_cp_preserve_2d_offline_gpu_legacy(const T * U, const T * V,
...
static void cp_cuda_check(cudaError_t status, const char* operation)
{
...
struct CpCompressionWorkspace2D {
    CucpszCodec codec = CucpszCodec::Huffman;
    size_t n = 0;
    size_t tile_count = 0;
    size_t mask_word_count = 0;
    size_t zeroeb_mask_bytes = 0;
    size_t land_bitpack_capacity = 0;

    float* dU = nullptr;
    float* dV = nullptr;
    uint8_t* eq_dEb_V = nullptr;
    uint8_t* tile_eq_dEb_U = nullptr;
    uint8_t* tile_eq_dEb_V = nullptr;
    uint32_t* zeroeb_U_word_offsets = nullptr;
    uint32_t* zeroeb_V_word_offsets = nullptr;
    uint32_t* zeroeb_mask_U = nullptr;
    uint32_t* zeroeb_mask_V = nullptr;
    uint32_t* land_mask = nullptr;
    float* zeroeb_val_U = nullptr;
    float* zeroeb_val_V = nullptr;
    uint32_t* zeroeb_count_U = nullptr;
    uint32_t* zeroeb_count_V = nullptr;
    uint32_t* land_count = nullptr;
    void* scan_temp = nullptr;
    size_t scan_temp_bytes = 0;

    // Distinct pairs per pre-codec stage (rather than one pair reused four
    // times) so all four can be recorded back-to-back without an
    // intermediate cudaEventSynchronize; their elapsed times are only read
    // once, after the codec stage has already forced the stream to drain.
    cudaEvent_t stage_start = nullptr;
    cudaEvent_t stage_stop = nullptr;
    cudaEvent_t stage2_start = nullptr;
    cudaEvent_t stage2_stop = nullptr;
    cudaEvent_t stage3_start = nullptr;
    cudaEvent_t stage3_stop = nullptr;
    cudaEvent_t stage4_start = nullptr;
    cudaEvent_t stage4_stop = nullptr;
    cudaEvent_t e2e_start = nullptr;
    cudaEvent_t e2e_stop = nullptr;
    GpuHuffmanU2Workspace* huffman = nullptr;
    GpuAnsU2Workspace* ans = nullptr;

    CpCompressionWorkspace2D(size_t r1, size_t r2, CucpszCodec selected_codec)
        : codec(selected_codec)
    {
        if (r1 == 0 || r2 == 0 || r1 > SIZE_MAX / r2) {
            throw std::invalid_argument("Invalid compression workspace dimensions");
        }
        n = r1 * r2;
        const size_t tile_cols = (r2 + LORENZO_TILE_DIM - 1) / LORENZO_TILE_DIM;
        const size_t tile_rows = (r1 + LORENZO_TILE_DIM - 1) / LORENZO_TILE_DIM;
        tile_count = tile_cols * tile_rows;
        mask_word_count = (n + 31) / 32;
        zeroeb_mask_bytes = mask_word_count * sizeof(uint32_t);
        land_bitpack_capacity = (n + 7) / 8;
        ...
        try {
            cp_cuda_check(cudaMalloc(&dU, n * sizeof(float)), "allocate device U");
            cp_cuda_check(cudaMalloc(&dV, n * sizeof(float)), "allocate device V");
            cp_cuda_check(cudaMalloc(&eq_dEb_V, n * sizeof(uint8_t)), "allocate V EB IDs");
            cp_cuda_check(cudaMalloc(&tile_eq_dEb_U, tile_count * sizeof(uint8_t)),
                          "allocate U tile EB IDs");
            cp_cuda_check(cudaMalloc(&tile_eq_dEb_V, tile_count * sizeof(uint8_t)),
                          "allocate V tile EB IDs");
            cp_cuda_check(cudaMalloc(&zeroeb_U_word_offsets, mask_word_count * sizeof(uint32_t)),
                          "allocate U zeroEB offsets");
            cp_cuda_check(cudaMalloc(&zeroeb_V_word_offsets, mask_word_count * sizeof(uint32_t)),
                          "allocate V zeroEB offsets");
            cp_cuda_check(cudaMalloc(&zeroeb_mask_U, zeroeb_mask_bytes),
                          "allocate U zeroEB mask");
            cp_cuda_check(cudaMalloc(&zeroeb_mask_V, zeroeb_mask_bytes),
                          "allocate V zeroEB mask");
            cp_cuda_check(cudaMalloc(&land_mask, zeroeb_mask_bytes), "allocate land mask");

            // Full-capacity arrays remove the data-dependent cudaMalloc from compression.
            cp_cuda_check(cudaMalloc(&zeroeb_val_U, n * sizeof(float)),
                          "allocate U zeroEB values");
            cp_cuda_check(cudaMalloc(&zeroeb_val_V, n * sizeof(float)),
                          "allocate V zeroEB values");
            cp_cuda_check(cudaMalloc(&zeroeb_count_U, sizeof(uint32_t)),
                          "allocate U zeroEB count");
            cp_cuda_check(cudaMalloc(&zeroeb_count_V, sizeof(uint32_t)),
                          "allocate V zeroEB count");
            cp_cuda_check(cudaMalloc(&land_count, sizeof(uint32_t)), "allocate land count");

            cp_cuda_check(cub::DeviceScan::ExclusiveSum(
                              nullptr, scan_temp_bytes,
                              zeroeb_U_word_offsets, zeroeb_U_word_offsets,
                              static_cast<int>(mask_word_count)),
                          "query CUB scan workspace");
            cp_cuda_check(cudaMalloc(&scan_temp, scan_temp_bytes), "allocate CUB scan workspace");
            cp_cuda_check(cudaEventCreate(&stage_start), "create stage start event");
            cp_cuda_check(cudaEventCreate(&stage_stop), "create stage stop event");
            cp_cuda_check(cudaEventCreate(&stage2_start), "create stage2 start event");
            cp_cuda_check(cudaEventCreate(&stage2_stop), "create stage2 stop event");
            cp_cuda_check(cudaEventCreate(&stage3_start), "create stage3 start event");
            cp_cuda_check(cudaEventCreate(&stage3_stop), "create stage3 stop event");
            cp_cuda_check(cudaEventCreate(&stage4_start), "create stage4 start event");
            cp_cuda_check(cudaEventCreate(&stage4_stop), "create stage4 stop event");
            cp_cuda_check(cudaEventCreate(&e2e_start), "create E2E start event");
            cp_cuda_check(cudaEventCreate(&e2e_stop), "create E2E stop event");
            if (codec == CucpszCodec::Huffman) {
                huffman = create_gpu_huffman_u2_workspace(n);
            }
            else {
                ans = create_gpu_ans_u2_workspace(n, nullptr);
            }
        }
        catch (...) {
            release();
    ...
    CpCompressionWorkspace2D(const CpCompressionWorkspace2D&) = delete;
    CpCompressionWorkspace2D& operator=(const CpCompressionWorkspace2D&) = delete;

    ~CpCompressionWorkspace2D() { release(); }

private:
    void release()
    {
        if (ans) destroy_gpu_ans_u2_workspace(ans);
        ans = nullptr;
        if (huffman) destroy_gpu_huffman_u2_workspace(huffman);
        huffman = nullptr;
        if (e2e_start) cudaEventDestroy(e2e_start);
        if (e2e_stop) cudaEventDestroy(e2e_stop);
        if (stage_start) cudaEventDestroy(stage_start);
        if (stage_stop) cudaEventDestroy(stage_stop);
        if (stage2_start) cudaEventDestroy(stage2_start);
        if (stage2_stop) cudaEventDestroy(stage2_stop);
        if (stage3_start) cudaEventDestroy(stage3_start);
        if (stage3_stop) cudaEventDestroy(stage3_stop);
        if (stage4_start) cudaEventDestroy(stage4_start);
        if (stage4_stop) cudaEventDestroy(stage4_stop);
        e2e_start = e2e_stop = stage_start = stage_stop = nullptr;
        stage2_start = stage2_stop = stage3_start = stage3_stop = nullptr;
        stage4_start = stage4_stop = nullptr;
        if (scan_temp) cudaFree(scan_temp);
        if (land_count) cudaFree(land_count);
        if (zeroeb_count_V) cudaFree(zeroeb_count_V);
        if (zeroeb_count_U) cudaFree(zeroeb_count_U);
        if (zeroeb_val_V) cudaFree(zeroeb_val_V);
        if (zeroeb_val_U) cudaFree(zeroeb_val_U);
        if (land_mask) cudaFree(land_mask);
        if (zeroeb_mask_V) cudaFree(zeroeb_mask_V);
        if (zeroeb_mask_U) cudaFree(zeroeb_mask_U);
        if (zeroeb_V_word_offsets) cudaFree(zeroeb_V_word_offsets);
        if (zeroeb_U_word_offsets) cudaFree(zeroeb_U_word_offsets);
        if (tile_eq_dEb_V) cudaFree(tile_eq_dEb_V);
        if (tile_eq_dEb_U) cudaFree(tile_eq_dEb_U);
        if (eq_dEb_V) cudaFree(eq_dEb_V);
        if (dV) cudaFree(dV);
        if (dU) cudaFree(dU);
        scan_temp = nullptr;
        land_count = zeroeb_count_V = zeroeb_count_U = nullptr;
        zeroeb_val_V = zeroeb_val_U = nullptr;
        land_mask = zeroeb_mask_V = zeroeb_mask_U = nullptr;
        zeroeb_V_word_offsets = zeroeb_U_word_offsets = nullptr;
        tile_eq_dEb_V = tile_eq_dEb_U = eq_dEb_V = nullptr;
        dV = dU = nullptr;
    }
};

struct CpCompressedDeviceResult2D {
    CucpszCodec codec = CucpszCodec::Huffman;
    size_t r1 = 0;
    size_t r2 = 0;
    float max_pwr_eb = 0.0f;
    uint8_t* blobs[4] = {nullptr, nullptr, nullptr, nullptr};
    size_t blob_lens[4] = {0, 0, 0, 0};
    uint32_t* outlier_count_U = nullptr;
    uint32_t* outlier_count_V = nullptr;
    uint32_t* outlier_idx_U = nullptr;
    uint32_t* outlier_idx_V = nullptr;
    float* outlier_val_U = nullptr;
    float* outlier_val_V = nullptr;
    uint32_t* zeroeb_count_U = nullptr;
    uint32_t* zeroeb_count_V = nullptr;
    uint32_t* zeroeb_mask_U = nullptr;
    uint32_t* zeroeb_mask_V = nullptr;
    ...
    size_t mask_bytes = 0;
    size_t land_bitpack_capacity = 0;
    GpuAnsU2Workspace* ans_workspace = nullptr;
};

// GPU-resident codec path. All allocations and input H2D transfers are owned by
// the caller; on return every compressed segment still resides on the GPU.
// this version increase the E2E throughput, which defined in cuszp paper.
static void sz_compress_cp_preserve_2d_offline_gpu(
    const float* dU, const float* dV,
    uint16_t* eq_U, uint16_t* eq_V, uint8_t* eq_dEb_U,
    size_t r1, size_t r2, float max_pwr_eb,
    float* ot_val_U, uint32_t* ot_idx_U, uint32_t* ot_num_U,
    float* ot_val_V, uint32_t* ot_idx_V, uint32_t* ot_num_V,
    CpCompressionWorkspace2D& workspace,
    CpCompressedDeviceResult2D& result,
    CucpszCodec codec,
    const CpCompressDebugOptions& debug_options)
{
    const size_t n = r1 * r2;
    if (workspace.n != n) throw std::invalid_argument("Compression workspace size mismatch");
    if (!debug_options.run_derive_eb || !debug_options.run_quantization_lorenzo ||
        !debug_options.run_hf || !debug_options.use_tile_uniform_eb) {
        throw std::invalid_argument(
            "GPU-resident compression requires derive EB, tile Lorenzo, and entropy coding");
    }
    if (workspace.codec != codec) {
        throw std::invalid_argument("Compression workspace codec mismatch");
    }
    if (debug_options.verify_derive_eb || debug_options.write_debug_data) {
        throw std::invalid_argument(
            "Host debug downloads are disabled in the GPU-resident compression path");
    }

    const float threshold = 1.0f / (1 << 20);
    const size_t tile_cols = (r2 + LORENZO_TILE_DIM - 1) / LORENZO_TILE_DIM;
    const size_t tile_rows = (r1 + LORENZO_TILE_DIM - 1) / LORENZO_TILE_DIM;
    dim3 derive_block(BLOCKSIZE_X, BLOCKSIZE_Y, 1);
    dim3 derive_grid(
        (r2 + derive_block.x - 3) / (derive_block.x - 2),
        (r1 + derive_block.y - 3) / (derive_block.y - 2));
    float ms_derive_eb = 0.0f;
    float ms_special_classify = 0.0f;
    float ms_word_scan = 0.0f;
    float ms_zero_compact = 0.0f;
    float ms_lrz = 0.0f;
    float hf_encode_ms[2] = {0.0f, 0.0f};
    float hf_wall_ms = 0.0f;
    float hf_kernel_ms = 0.0f;
    uint8_t* hf_device_blobs[2] = {nullptr, nullptr};
    size_t hf_lens[2] = {0, 0};
    float ans_kernel_ms = 0.0f;
    float ans_device_ms = 0.0f;

    printf("\nRUN_DERIVE_EB\n");
    printf("gridSize_v2: %u, %u\n", derive_grid.x, derive_grid.y);
    cudaEventRecord(workspace.stage_start);
    derive_eb_offline_v2<<<derive_grid, derive_block>>>(
        dU, dV, eq_dEb_U, workspace.eq_dEb_V, r1, r2, max_pwr_eb);
    cudaEventRecord(workspace.stage_stop);

    printf("FUSED_LAND_EBZERO_TILE (tile=%dx%d)\n",
           LORENZO_TILE_DIM, LORENZO_TILE_DIM);
    const int replace_id = static_cast<int>(log2(max_pwr_eb / threshold) / 2.0);
    cudaMemset(workspace.zeroeb_mask_U, 0, workspace.zeroeb_mask_bytes);
    cudaMemset(workspace.zeroeb_mask_V, 0, workspace.zeroeb_mask_bytes);
    cudaMemset(workspace.land_mask, 0, workspace.zeroeb_mask_bytes);
    cudaMemset(workspace.zeroeb_count_U, 0, sizeof(uint32_t));
    cudaMemset(workspace.zeroeb_count_V, 0, sizeof(uint32_t));
    cudaMemset(workspace.land_count, 0, sizeof(uint32_t));

    dim3 special_block(32, 8, 1);
    dim3 special_grid(
        static_cast<unsigned int>(tile_cols),
        static_cast<unsigned int>(tile_rows), 1);
    cudaEventRecord(workspace.stage2_start);
    kernel_classify_specials_and_tile_eb<float, uint8_t>
        <<<special_grid, special_block>>>(
            dU, dV, eq_dEb_U, workspace.eq_dEb_V,
            workspace.zeroeb_mask_U, workspace.zeroeb_mask_V,
            workspace.land_mask,
            workspace.tile_eq_dEb_U, workspace.tile_eq_dEb_V,
            static_cast<uint8_t>(replace_id), r1, r2,
            workspace.mask_word_count,
            debug_options.deal_with_land_data,
            debug_options.deal_with_ebzero);
    cudaEventRecord(workspace.stage2_stop);

    constexpr int word_block = 256;
    const int word_grid = static_cast<int>(
        (workspace.mask_word_count + word_block - 1) / word_block);
    cudaEventRecord(workspace.stage3_start);
    kernel_mask_word_counts_pair_and_land<<<word_grid, word_block>>>(
        workspace.zeroeb_mask_U, workspace.zeroeb_mask_V,
        workspace.land_mask,
        workspace.zeroeb_U_word_offsets, workspace.zeroeb_V_word_offsets,
        workspace.land_count, workspace.mask_word_count);
    cp_cuda_check(cub::DeviceScan::ExclusiveSum(
                      workspace.scan_temp, workspace.scan_temp_bytes,
                      workspace.zeroeb_U_word_offsets,
                      workspace.zeroeb_U_word_offsets,
                      static_cast<int>(workspace.mask_word_count)),
                  "scan U zeroEB word counts");
    cp_cuda_check(cub::DeviceScan::ExclusiveSum(
                      workspace.scan_temp, workspace.scan_temp_bytes,
                      workspace.zeroeb_V_word_offsets,
                      workspace.zeroeb_V_word_offsets,
                      static_cast<int>(workspace.mask_word_count)),
                  "scan V zeroEB word counts");
    kernel_finalize_zeroeb_counts<<<1, 1>>>(
        workspace.zeroeb_mask_U, workspace.zeroeb_mask_V,
        workspace.zeroeb_U_word_offsets, workspace.zeroeb_V_word_offsets,
        workspace.zeroeb_count_U, workspace.zeroeb_count_V,
        workspace.mask_word_count);
    cudaEventRecord(workspace.stage3_stop);

    constexpr int compact_block = 256;
    constexpr int compact_warps = compact_block / 32;
    const int compact_grid = static_cast<int>(
        (workspace.mask_word_count + compact_warps - 1) / compact_warps);
    cudaEventRecord(workspace.stage4_start);
    kernel_compact_zeroeb_values_pair_by_word<float>
        <<<compact_grid, compact_block>>>(
            dU, dV,
            workspace.zeroeb_mask_U, workspace.zeroeb_mask_V,
            workspace.zeroeb_U_word_offsets, workspace.zeroeb_V_word_offsets,
            workspace.zeroeb_val_U, workspace.zeroeb_val_V,
            n, workspace.mask_word_count);
    cudaEventRecord(workspace.stage4_stop);
    // Elapsed times for all four stages above are read after the Lorenzo and
    // codec stages below, which already force the stream to drain (Huffman's
    // encode_timed and ANS's compress() both block internally) — so no extra
    // cudaEventSynchronize is needed here to get accurate stage timings.

    printf("RUN_QUANTIZATION_LORENZO\n");
    cudaMemset(ot_num_U, 0, sizeof(uint32_t));
    cudaMemset(ot_num_V, 0, sizeof(uint32_t));
    cudaMemset(eq_U, 0, n * sizeof(uint16_t));
    cudaMemset(eq_V, 0, n * sizeof(uint16_t));
    float lrz_U_ms = 0.0f, lrz_V_ms = 0.0f;
    psz::cuhip::GPU_PROTO_c_lorenzo_nd_with_outlier__bypass_outlier_struct__tile_eb
        <float, uint16_t, uint8_t>(
            const_cast<float*>(dU), dim3(r2, r1, 1),
            eq_U, ot_val_U, ot_idx_U, ot_num_U,
            workspace.tile_eq_dEb_U, threshold, RADIUS, &lrz_U_ms, 0);
    psz::cuhip::GPU_PROTO_c_lorenzo_nd_with_outlier__bypass_outlier_struct__tile_eb
        <float, uint16_t, uint8_t>(
            const_cast<float*>(dV), dim3(r2, r1, 1),
            eq_V, ot_val_V, ot_idx_V, ot_num_V,
            workspace.tile_eq_dEb_V, threshold, RADIUS, &lrz_V_ms, 0);
    ms_lrz = lrz_U_ms + lrz_V_ms;

    if (codec == CucpszCodec::Huffman) {
        run_gpu_huffman_u2_arrays_device(
            workspace.huffman, eq_U, eq_V, n, nullptr,
            hf_lens, hf_device_blobs, hf_encode_ms,
            &hf_wall_ms, &hf_kernel_ms);
    }
    else {
        printf("RUN_ANS (device-resident output)\n");
        auto ans_start = std::chrono::steady_clock::now();
        run_gpu_ans_u2_arrays_device(
            workspace.ans, eq_U, eq_V, n, &ans_kernel_ms);
        ans_device_ms = std::chrono::duration<float, std::milli>(
            std::chrono::steady_clock::now() - ans_start).count();
    }

    // Lorenzo and the codec call above both block internally, so the whole
    // stream — including the four pre-codec stages recorded earlier — is
    // guaranteed complete here; read all four stage timings in one place.
    cudaEventElapsedTime(&ms_derive_eb, workspace.stage_start, workspace.stage_stop);
    cudaEventElapsedTime(
        &ms_special_classify, workspace.stage2_start, workspace.stage2_stop);
    cudaEventElapsedTime(&ms_word_scan, workspace.stage3_start, workspace.stage3_stop);
    cudaEventElapsedTime(&ms_zero_compact, workspace.stage4_start, workspace.stage4_stop);

    result.codec = codec;
    result.r1 = r1;
    result.r2 = r2;
    result.max_pwr_eb = max_pwr_eb;
    result.blobs[0] = hf_device_blobs[0];
    result.blobs[1] = hf_device_blobs[1];
    result.blobs[2] = workspace.tile_eq_dEb_U;
    result.blobs[3] = workspace.tile_eq_dEb_V;
    result.blob_lens[0] = hf_lens[0];
    result.blob_lens[1] = hf_lens[1];
    result.blob_lens[2] = workspace.tile_count * sizeof(uint8_t);
    result.blob_lens[3] = workspace.tile_count * sizeof(uint8_t);
    result.ans_workspace = workspace.ans;
    result.outlier_count_U = ot_num_U;
    result.outlier_count_V = ot_num_V;
    result.outlier_idx_U = ot_idx_U;
    result.outlier_idx_V = ot_idx_V;
    result.outlier_val_U = ot_val_U;
    result.outlier_val_V = ot_val_V;
    result.zeroeb_count_U = workspace.zeroeb_count_U;
    result.zeroeb_count_V = workspace.zeroeb_count_V;
    result.zeroeb_mask_U = workspace.zeroeb_mask_U;
    result.zeroeb_mask_V = workspace.zeroeb_mask_V;
    result.zeroeb_val_U = workspace.zeroeb_val_U;
    result.zeroeb_val_V = workspace.zeroeb_val_V;
    result.land_count = workspace.land_count;
    result.land_mask = workspace.land_mask;
    result.mask_bytes = workspace.zeroeb_mask_bytes;
    result.land_bitpack_capacity = workspace.land_bitpack_capacity;

    const double input_bytes = static_cast<double>(n) * 2.0 * sizeof(float);
    const double gib = 1024.0 * 1024.0 * 1024.0;
    const float codec_kernel_ms = codec == CucpszCodec::Huffman
        ? hf_kernel_ms : ans_kernel_ms;
    const float kernel_total = ms_derive_eb + ms_special_classify +
                               ms_word_scan + ms_zero_compact + ms_lrz +
                               codec_kernel_ms;
    printf("COMPRESS_TIME (GPU kernels only)\n");
    printf("  derive_eb  : %8.3f ms  (derive per-point EB IDs)\n", ms_derive_eb);
    printf("  special_cls: %8.3f ms  (land/zeroEB masks + tile minimum EB)\n",
           ms_special_classify);
    printf("  word_scan  : %8.3f ms  (mask popcount + CUB U/V prefix scans)\n",
           ms_word_scan);
    printf("  zero_pack  : %8.3f ms  (ordered U/V zeroEB compaction)\n",
           ms_zero_compact);
    printf("  lorenzo    : %8.3f ms  (U/V tile quantization)\n", ms_lrz);
    if (codec == CucpszCodec::Huffman) {
        printf("  huffman    : %8.3f ms  (U/V histogram + encode kernels)\n",
               hf_kernel_ms);
    }
    ...
    printf("  total      : %8.3f ms  (%.2f GiB/s)\n",
           kernel_total, input_bytes / gib / (kernel_total / 1000.0));
    if (codec == CucpszCodec::Huffman) {
        printf("  huffman-E2E: %8.3f ms  (CPU codebook + GPU + small metadata copies; excluded above)\n\n",
               hf_wall_ms);
    }
    else {
        printf("  ans-call   : %8.3f ms  (launch + GPU + sync; ans kernels already included in total)\n\n",
               ans_device_ms);
    }
}

static double download_and_write_cucpsz(
    const char* fname,
    const CpCompressedDeviceResult2D& result,
    double* out_download_seconds)
{
    using Clock = std::chrono::steady_clock;
    auto download_start = Clock::now();
    uint32_t ot_count_U = 0, ot_count_V = 0;
    uint32_t zeroeb_count_U = 0, zeroeb_count_V = 0;
    uint32_t land_count = 0;
    cudaMemcpy(&ot_count_U, result.outlier_count_U, sizeof(uint32_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(&ot_count_V, result.outlier_count_V, sizeof(uint32_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(&zeroeb_count_U, result.zeroeb_count_U, sizeof(uint32_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(&zeroeb_count_V, result.zeroeb_count_V, sizeof(uint32_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(&land_count, result.land_count, sizeof(uint32_t), cudaMemcpyDeviceToHost);

    uint8_t* h_blobs[4] = {nullptr, nullptr, nullptr, nullptr};
    size_t h_blob_lens[4] = {
        result.blob_lens[0], result.blob_lens[1],
        result.blob_lens[2], result.blob_lens[3]};
    int first_direct_blob = 0;
    if (result.codec == CucpszCodec::Ans) {
        download_gpu_ans_u2_arrays(
            result.ans_workspace, h_blobs, h_blob_lens);
        first_direct_blob = 2;
    }
    for (int i = first_direct_blob; i < 4; ++i) {
        h_blobs[i] = new uint8_t[h_blob_lens[i] ? h_blob_lens[i] : 1];
        if (h_blob_lens[i]) {
            cudaMemcpy(h_blobs[i], result.blobs[i], h_blob_lens[i], cudaMemcpyDeviceToHost);
        }
    }

    uint32_t* h_ot_idx_U = new uint32_t[ot_count_U ? ot_count_U : 1];
    uint32_t* h_ot_idx_V = new uint32_t[ot_count_V ? ot_count_V : 1];
    float* h_ot_val_U = new float[ot_count_U ? ot_count_U : 1];
    float* h_ot_val_V = new float[ot_count_V ? ot_count_V : 1];
    uint8_t* h_zeroeb_mask_U = new uint8_t[zeroeb_count_U ? result.mask_bytes : 1];
    uint8_t* h_zeroeb_mask_V = new uint8_t[zeroeb_count_V ? result.mask_bytes : 1];
    float* h_zeroeb_val_U = new float[zeroeb_count_U ? zeroeb_count_U : 1];
    float* h_zeroeb_val_V = new float[zeroeb_count_V ? zeroeb_count_V : 1];
    if (ot_count_U) {
        cudaMemcpy(h_ot_idx_U, result.outlier_idx_U,
                   ot_count_U * sizeof(uint32_t), cudaMemcpyDeviceToHost);
        cudaMemcpy(h_ot_val_U, result.outlier_val_U,
                   ot_count_U * sizeof(float), cudaMemcpyDeviceToHost);
    }
    if (ot_count_V) {
        cudaMemcpy(h_ot_idx_V, result.outlier_idx_V,
                   ot_count_V * sizeof(uint32_t), cudaMemcpyDeviceToHost);
        cudaMemcpy(h_ot_val_V, result.outlier_val_V,
                   ot_count_V * sizeof(float), cudaMemcpyDeviceToHost);
    }
    if (zeroeb_count_U) {
        cudaMemcpy(h_zeroeb_mask_U, result.zeroeb_mask_U,
                   result.mask_bytes, cudaMemcpyDeviceToHost);
        cudaMemcpy(h_zeroeb_val_U, result.zeroeb_val_U,
                   zeroeb_count_U * sizeof(float), cudaMemcpyDeviceToHost);
    }
    if (zeroeb_count_V) {
        cudaMemcpy(h_zeroeb_mask_V, result.zeroeb_mask_V,
                   result.mask_bytes, cudaMemcpyDeviceToHost);
        cudaMemcpy(h_zeroeb_val_V, result.zeroeb_val_V,
                   zeroeb_count_V * sizeof(float), cudaMemcpyDeviceToHost);
    }
    const size_t land_bytes = land_count ? result.land_bitpack_capacity : 0;
    uint8_t* h_land = new uint8_t[land_bytes ? land_bytes : 1];
    if (land_bytes) {
        cudaMemcpy(h_land, result.land_mask, land_bytes, cudaMemcpyDeviceToHost);
    }
    const double download_seconds =
        std::chrono::duration<double>(Clock::now() - download_start).count();
    if (out_download_seconds) *out_download_seconds = download_seconds;

    printf("COMPRESSED_RESULT\n");
    printf("  outlier_U=%u  zeroeb_U=%u  outlier_V=%u  zeroeb_V=%u  land=%u\n",
           ot_count_U, zeroeb_count_U, ot_count_V, zeroeb_count_V, land_count);
    double write_seconds = write_cucpsz(
        fname, result.r1, result.r2, result.max_pwr_eb,
        h_blobs, h_blob_lens,
        ot_count_U, h_ot_idx_U, h_ot_val_U,
        zeroeb_count_U, h_zeroeb_mask_U, h_zeroeb_val_U,
        ot_count_V, h_ot_idx_V, h_ot_val_V,
        zeroeb_count_V, h_zeroeb_mask_V, h_zeroeb_val_V,
        h_land, land_bytes, result.codec,
        1u, static_cast<uint32_t>(LORENZO_TILE_DIM));

    const size_t original_field_bytes = result.r1 * result.r2 * sizeof(float);
    const size_t special_U = ot_count_U * (sizeof(uint32_t) + sizeof(float)) +
                             (zeroeb_count_U ? result.mask_bytes : 0) +
                             zeroeb_count_U * sizeof(float);
    const size_t special_V = ot_count_V * (sizeof(uint32_t) + sizeof(float)) +
                             (zeroeb_count_V ? result.mask_bytes : 0) +
                             zeroeb_count_V * sizeof(float);
    const size_t compressed_U = h_blob_lens[0] + h_blob_lens[2] + special_U;
    const size_t compressed_V = h_blob_lens[1] + h_blob_lens[3] + special_V;
    const size_t compressed_file = sizeof(CucpszHeader) + land_bytes +
                                   compressed_U + compressed_V;
    printf("COMPUTE_RATIO\n");
    printf("U compression ratio: %f\n", (double)original_field_bytes / compressed_U);
    printf("V compression ratio: %f\n", (double)original_field_bytes / compressed_V);
    printf("Overall compression ratio: %f\n\n",
           (double)(2 * original_field_bytes) / compressed_file);

    for (auto& blob : h_blobs) delete[] blob;
    delete[] h_ot_idx_U;
    delete[] h_ot_idx_V;
    delete[] h_ot_val_U;
    delete[] h_ot_val_V;
    delete[] h_zeroeb_mask_U;
    delete[] h_zeroeb_mask_V;
    delete[] h_zeroeb_val_U;
    delete[] h_zeroeb_val_V;
    delete[] h_land;
    return write_seconds;
}


// Runs one full untimed compression on a tiny synthetic field before the
// real, timed compression. This absorbs CUDA context init, kernel/module
// JIT, and GPU clock ramp-up so GPU_RESIDENT_E2E reflects steady-state
// throughput instead of first-launch cold start.
static double warmup_offline_compress(CucpszCodec codec)
{
    using WarmClock = std::chrono::steady_clock;
    const auto warm_start = WarmClock::now();
    constexpr size_t warm_r1 = 64, warm_r2 = 64;
    constexpr size_t warm_n = warm_r1 * warm_r2;
    std::vector<float> h_u(warm_n), h_v(warm_n);
    for (size_t i = 0; i < warm_n; ++i) {
        h_u[i] = 1.0f + 0.5f * sinf(static_cast<float>(i) * 0.037f);
        h_v[i] = 1.0f + 0.5f * cosf(static_cast<float>(i) * 0.053f);
    }

    OtData ot_U, ot_V;
    allocOtData(ot_U, warm_n);
    allocOtData(ot_V, warm_n);
    uint16_t *eq_U = nullptr, *eq_V = nullptr;
    uint8_t *eq_dEb = nullptr;
    cudaMalloc(&eq_U, warm_n * sizeof(uint16_t));
    cudaMalloc(&eq_V, warm_n * sizeof(uint16_t));
    cudaMalloc(&eq_dEb, warm_n * sizeof(uint8_t));
    {
        CpCompressionWorkspace2D workspace(warm_r1, warm_r2, codec);
        cudaMemcpy(workspace.dU, h_u.data(), warm_n * sizeof(float), cudaMemcpyHostToDevice);
        cudaMemcpy(workspace.dV, h_v.data(), warm_n * sizeof(float), cudaMemcpyHostToDevice);
        cudaDeviceSynchronize();
        CpCompressedDeviceResult2D device_result;
        CpCompressDebugOptions debug_options;
        sz_compress_cp_preserve_2d_offline_gpu(
            workspace.dU, workspace.dV,
            eq_U, eq_V, eq_dEb, warm_r1, warm_r2, 0.1f,
            ot_U.val, ot_U.idx, ot_U.num,
            ot_V.val, ot_V.idx, ot_V.num,
            workspace, device_result, codec, debug_options);
        cudaDeviceSynchronize();
    }
    cudaFree(eq_dEb);
    cudaFree(eq_V);
...
int main(int argc, char ** argv){
    ...
    printf("=== GPU warmup (untimed, synthetic 64x64 field) ===\n");
    const double warmup_seconds = warmup_offline_compress(codec);
    ...
    double input_h2d_seconds = -1.0;
    double result_d2h_seconds = -1.0;
    try {
        CpCompressionWorkspace2D workspace(r1, r2, codec);

        auto h2d_start = IoClock::now();
        cp_cuda_check(cudaMemcpy(
                          workspace.dU, U, expected_elements * sizeof(float),
                          cudaMemcpyHostToDevice),
                      "copy U to GPU");
        cp_cuda_check(cudaMemcpy(
                          workspace.dV, V, expected_elements * sizeof(float),
                          cudaMemcpyHostToDevice),
                      "copy V to GPU");
        cp_cuda_check(cudaDeviceSynchronize(), "finish input preparation");
        input_h2d_seconds =
            std::chrono::duration<double>(IoClock::now() - h2d_start).count();

        // Host input is no longer part of the timed or output phases.
        free(U);
        free(V);
        U = V = nullptr;

        CpCompressedDeviceResult2D device_result;
        cp_cuda_check(cudaEventRecord(workspace.e2e_start), "record E2E start");
        auto resident_start = IoClock::now();
        sz_compress_cp_preserve_2d_offline_gpu(
            workspace.dU, workspace.dV,
            eq_U, eq_V, eq_dEb, r1, r2, max_eb,
            ot_U.val, ot_U.idx, ot_U.num,
            ot_V.val, ot_V.idx, ot_V.num,
            workspace, device_result, codec, debug_options);
        cp_cuda_check(cudaEventRecord(workspace.e2e_stop), "record E2E stop");
        cp_cuda_check(cudaEventSynchronize(workspace.e2e_stop), "finish GPU-resident compression");
        const double resident_wall_ms =
            std::chrono::duration<double, std::milli>(
                IoClock::now() - resident_start).count();
        float resident_event_ms = 0.0f;
        cp_cuda_check(cudaEventElapsedTime(
                          &resident_event_ms,
                          workspace.e2e_start, workspace.e2e_stop),
                      "measure GPU-resident compression");
        const double input_gib =
            expected_elements * 2.0 * sizeof(float) /
            (1024.0 * 1024.0 * 1024.0);
        printf("GPU_RESIDENT_E2E (warmed up with RANDOM dataset)\n");
        printf("  boundary   : device inputs/workspace ready -> compressed device segments ready\n");
        printf("  wall       : %8.3f ms  (%.2f GiB/s)\n",
               resident_wall_ms, input_gib / (resident_wall_ms / 1000.0));
        printf("  CUDA event : %8.3f ms  (same device-ready boundary)\n\n",
               resident_event_ms);

        file_write_seconds = download_and_write_cucpsz(
            cucpsz_fname.c_str(), device_result, &result_d2h_seconds);
    }
    ...
    cudaFree(eq_V);
    cudaFree(eq_dEb);
    if (input_h2d_seconds >= 0.0 || result_d2h_seconds >= 0.0) {
        printf("GPU_TRANSFER_TIME (excluded from GPU_RESIDENT_E2E)\n");
        if (input_h2d_seconds >= 0.0) {
            printf("  input H2D : %.6f s\n", input_h2d_seconds);
        }
        if (result_d2h_seconds >= 0.0) {
            printf("  result D2H: %.6f s\n", result_d2h_seconds);
        }
        printf("\n");
    }
    printf("FILE_IO_TIME\n");

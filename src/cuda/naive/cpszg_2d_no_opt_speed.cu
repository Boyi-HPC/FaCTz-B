...
struct CucpszHeader
{
    ...
    uint64_t hf_blob_len[4];                 // eq_U, eq_V, eq_dEb_U, eq_dEb_V
    ...
    const uint8_t *land_bitpack, size_t land_bitpack_bytes)
{
    ...
    hdr.ot_count_U = ot_count_U;
    hdr.ot_count_V = ot_count_V;
    hdr.zeroeb_count_U = zeroeb_count_U;
    hdr.zeroeb_count_V = zeroeb_count_V;
    for (int i = 0; i < 4; i++)
        hdr.hf_blob_len[i] = hf_lens[i];
    ...
    { return bytes * 100.0 / total_bytes; };

    const char *hf_names[4] = {"eq_U", "eq_V", "eq_dEb_U", "eq_dEb_V"};
    for (int i = 0; i < 4; i++)
    {
        fwrite(hf_blobs[i], 1, hf_lens[i], f);
        printf("  HF blob %-10s : %zu bytes (%.1f%%)\n", hf_names[i], hf_lens[i], pct(hf_lens[i]));
    }
                                       ...
                                       const char *out_fname = nullptr)
{
    ...
    float ms_derive_eb = 0.0f, ms_land = 0.0f, ms_ebzero = 0.0f, ms_lrz = 0.0f;
    cudaEvent_t _ce0, _ce1;
    cudaEventCreate(&_ce0);
    cudaEventCreate(&_ce1);
    ...
    cudaMemset(d_land_bitpack, 0, land_bitpack_bytes);
    ...
    if (debug_options.run_derive_eb)
    {
        ...
        if (debug_options.deal_with_ebzero)
        {
            ...
            // U stencil on eq_dEb (U EB), V stencil on eq_dEb_V (V EB)
            ...
            // Now replace zeros and fix dEb arrays
        ...
        if (debug_options.run_cusz_lorenzo)
        {
            ...
            printf("\n");
            if (debug_options.test_cusz_lorenzo)
            {
        ...
        if (debug_options.run_quantization_lorenzo)
        {
            ...
            *h_ot_num_U = 0, *h_ot_num_V = 0;
            ...
            if (debug_options.run_hf)
            {
                ...
                cudaFree(decomp_buf_U);
                decomp_buf_U = nullptr;
                cudaFree(decomp_buf_V);
                decomp_buf_V = nullptr;
...

                {
                    ...
                    float total_ms = ms_derive_eb + ms_land + ms_ebzero + ms_lrz + hf_ms;
                ...
                if (out_fname && hf_blobs[0] && hf_blobs[1] && hf_blobs[2] && hf_blobs[3])
                {
                    ...
                    uint8_t *h_land_bitpack = new uint8_t[land_bitpack_bytes];
    ...
    T *dEb_U_dcomp;
    cudaMalloc(&dEb_U_dcomp, r1 * r2 * sizeof(T));
    T *dEb_V_dcomp;
    cudaMalloc(&dEb_V_dcomp, r1 * r2 * sizeof(T));
    thrust::for_each(
    ...
    T *dU_decomp;
    cudaMalloc(&dU_decomp, r1 * r2 * sizeof(T));
    ...
    // decompression V
    T *dV_decomp;
    cudaMalloc(&dV_decomp, r1 * r2 * sizeof(T));
    ...
    T *U_decomp, T *V_decomp)
{
    ...
    if (land_bp_bytes)
        fread(h_land_bitpack, 1, land_bp_bytes, f);
    ...
    T *dU_decomp;
    cudaMalloc(&dU_decomp, n * sizeof(T));
    ...
    // Decompression V
    T *dV_decomp;
    cudaMalloc(&dV_decomp, n * sizeof(T));
    ...
    cudaFree(d_ot_idx_U);
    cudaFree(d_ot_val_U);
    cudaFree(d_zeb_idx_U);
    cudaFree(d_zeb_val_U);
    cudaFree(d_ot_idx_V);
    cudaFree(d_ot_val_V);
    cudaFree(d_zeb_idx_V);
    cudaFree(d_zeb_val_V);
    cudaFree(d_dEb_U);
    cudaFree(d_dEb_V);
    cudaFree(dU_decomp);
    cudaFree(dV_decomp);
    ...
    std::unordered_map<size_t, CriticalPoint2D> &cps)
{
    ...
    for (int k = 0; k < 3; k++)
        if (v[k][0] == 0 && v[k][1] == 0)
            return;
    double mu[3], cond;
    if (!ftk::inverse_lerp_s2v2(v, mu, &cond, 0.0))
        return;
    double xp[2];
    ...
    if (delta >= 0)
    {
        if (eig[0].real() * eig[1].real() < 0)
            ...
            type = ftk::CRITICAL_POINT_2D_UNKNOWN;
    }
    else
    {
        if (eig[0].real() < 0)
    ...
    int r1, int r2, const char *label)
{
    ...
    for (auto &p : cps0)
        if (cps1.count(p.first))
            tp++;
        else
            fn++;
    for (auto &p : cps1)
        if (!cps0.count(p.first))
            fp++;
...
int main(int argc, char **argv)
{
    ...
    printf("  U: ");
    verify(U, U_decomp2, num_elements);
    printf("  V: ");
    verify(V, V_decomp2, num_elements);

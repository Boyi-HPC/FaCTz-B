...
static inline size_t bitmask_word_bytes(size_t n)
{
    ...
    uint32_t lorenzo_variant, uint32_t lorenzo_tile_dim)
{
    ...
    if (zeroeb_mask_bytes_U) fwrite(zeroeb_mask_U, 1, zeroeb_mask_bytes_U, f);
    ...
    if (zeroeb_mask_bytes_V) fwrite(zeroeb_mask_V, 1, zeroeb_mask_bytes_V, f);
...
sz_compress_cp_preserve_2d_offline_gpu(const T * U, const T * V,
    ...
    if (debug_options.use_tile_uniform_eb)
	    ...
	    {
	        constexpr int ZEB_RESTORE_BLK = 512;
	        int zeb_restore_grid = ((int)num_elements + ZEB_RESTORE_BLK - 1) / ZEB_RESTORE_BLK;
	        if (zero_eb_U_count)
	            kernel_restore_zeroeb_values<T><<<zeb_restore_grid, ZEB_RESTORE_BLK>>>(
	                d_zeroeb_mask_U, ebIsZero_U_indices, ebIsZero_U_data,
	                dU_decomp, num_elements);
	    }
    //decompression V
    ...
    if (debug_options.use_tile_uniform_eb)
	    ...
	    cudaDeviceSynchronize();
	    {
	        ...
	        if (zero_eb_V_count)
	            kernel_restore_zeroeb_values<T><<<zeb_restore_grid, ZEB_RESTORE_BLK>>>(
	                d_zeroeb_mask_V, ebIsZero_V_indices, ebisZero_V_data,
	                dV_decomp, num_elements);
	    }
        ...
        for (size_t i = 0; i < N; i++)
        {
            ...
            if (debug_options.use_tile_uniform_eb)
	            ...
	            if (zero_eb_U_count) {
	                constexpr int ZEB_RESTORE_BLK = 512;
	                int zeb_restore_grid = ((int)num_elements + ZEB_RESTORE_BLK - 1) / ZEB_RESTORE_BLK;
	                kernel_restore_zeroeb_values<T><<<zeb_restore_grid, ZEB_RESTORE_BLK, 0, stream>>>(
	                    d_zeroeb_mask_U, ebIsZero_U_indices, ebIsZero_U_data,
	                    dU_decomp, num_elements);
	            }
    ...
    if (err != cudaSuccess) {
        ...
        cudaFree(dEb_V);
	        ...
	        cudaFree(d_zeroeb_mask_U);
	        cudaFree(d_zeroeb_mask_V);
    ...
    T* U_decomp, T* V_decomp)
{
    ...
    for (int i = 0; i < 4; i++) {
	    ...
	    size_t zeroeb_mask_bytes_U = cnt_zeb_U ? bitmask_word_bytes(n) : 0;
	    size_t zeroeb_mask_bytes_V = cnt_zeb_V ? bitmask_word_bytes(n) : 0;

	    uint32_t* h_ot_idx_U  = new uint32_t[cnt_ot_U  ? cnt_ot_U  : 1];
	    float*    h_ot_val_U  = new float   [cnt_ot_U  ? cnt_ot_U  : 1];
	    uint8_t*  h_zeb_mask_U = new uint8_t[zeroeb_mask_bytes_U ? zeroeb_mask_bytes_U : 1];
	    float*    h_zeb_val_U = new float   [cnt_zeb_U ? cnt_zeb_U : 1];
	    uint32_t* h_ot_idx_V  = new uint32_t[cnt_ot_V  ? cnt_ot_V  : 1];
	    float*    h_ot_val_V  = new float   [cnt_ot_V  ? cnt_ot_V  : 1];
	    uint8_t*  h_zeb_mask_V = new uint8_t[zeroeb_mask_bytes_V ? zeroeb_mask_bytes_V : 1];
	    float*    h_zeb_val_V = new float   [cnt_zeb_V ? cnt_zeb_V : 1];
    ...
    if (land_bp_bytes) fread(h_land_bitpack, 1, land_bp_bytes, f);
	    ...
	    if (zeroeb_mask_bytes_U) fread(h_zeb_mask_U, 1, zeroeb_mask_bytes_U, f);
	    ...
	    if (zeroeb_mask_bytes_V) fread(h_zeb_mask_V, 1, zeroeb_mask_bytes_V, f);
    ...
    if (!tile_variant) {
	    ...
	    // Upload outlier arrays and zero-EB mask/value payloads to GPU.
	    uint32_t *d_ot_idx_U,  *d_ot_idx_V;
	    uint32_t *d_zeb_mask_U = nullptr, *d_zeb_mask_V = nullptr;
	    uint32_t *d_zeb_offsets = nullptr;
	    ...
	    if (cnt_zeb_U) cudaMalloc(&d_zeb_mask_U, zeroeb_mask_bytes_U);
	    if (cnt_zeb_V) cudaMalloc(&d_zeb_mask_V, zeroeb_mask_bytes_V);
	    if (cnt_zeb_U || cnt_zeb_V) cudaMalloc(&d_zeb_offsets, n * sizeof(uint32_t));
	    if (cnt_ot_U) {
	    ...
	    if (cnt_zeb_U) {
	        cudaMemcpy(d_zeb_mask_U, h_zeb_mask_U, zeroeb_mask_bytes_U, cudaMemcpyHostToDevice);
    ...
    if (cnt_ot_V) {
	    ...
	    if (cnt_zeb_V) {
	        cudaMemcpy(d_zeb_mask_V, h_zeb_mask_V, zeroeb_mask_bytes_V, cudaMemcpyHostToDevice);
    ...
    if (cnt_ot_U)
	    ...
	    if (cnt_zeb_U) {
	        constexpr int ZEB_RESTORE_BLK = 512;
	        int zeb_restore_grid = ((int)n + ZEB_RESTORE_BLK - 1) / ZEB_RESTORE_BLK;
	        kernel_mask_to_flags<<<zeb_restore_grid, ZEB_RESTORE_BLK>>>(
	            d_zeb_mask_U, d_zeb_offsets, n);
	        thrust::exclusive_scan(thrust::device,
	            d_zeb_offsets, d_zeb_offsets + n, d_zeb_offsets);
	        kernel_restore_zeroeb_values<T><<<zeb_restore_grid, ZEB_RESTORE_BLK>>>(
	            d_zeb_mask_U, d_zeb_offsets, d_zeb_val_U, dU_decomp, n);
	    }
    ...
    if (cnt_ot_V)
	    ...
	    if (cnt_zeb_V) {
	        ...
	        int zeb_restore_grid = ((int)n + ZEB_RESTORE_BLK - 1) / ZEB_RESTORE_BLK;
	        kernel_mask_to_flags<<<zeb_restore_grid, ZEB_RESTORE_BLK>>>(
	            d_zeb_mask_V, d_zeb_offsets, n);
	        thrust::exclusive_scan(thrust::device,
	            d_zeb_offsets, d_zeb_offsets + n, d_zeb_offsets);
	        kernel_restore_zeroeb_values<T><<<zeb_restore_grid, ZEB_RESTORE_BLK>>>(
	            d_zeb_mask_V, d_zeb_offsets, d_zeb_val_V, dV_decomp, n);
	    }
    ...
    if (d_eq_dEb_V) cudaFree(d_eq_dEb_V);
	    cudaFree(d_ot_idx_U); cudaFree(d_ot_val_U); cudaFree(d_zeb_val_U);
	    cudaFree(d_ot_idx_V); cudaFree(d_ot_val_V); cudaFree(d_zeb_val_V);
	    if (d_zeb_mask_U) cudaFree(d_zeb_mask_U);
	    if (d_zeb_mask_V) cudaFree(d_zeb_mask_V);
	    if (d_zeb_offsets) cudaFree(d_zeb_offsets);
    ...
    delete[] h_land_bitpack;
	    delete[] h_ot_idx_U; delete[] h_ot_val_U; delete[] h_zeb_mask_U; delete[] h_zeb_val_U;
	    delete[] h_ot_idx_V; delete[] h_ot_val_V; delete[] h_zeb_mask_V; delete[] h_zeb_val_V;
}

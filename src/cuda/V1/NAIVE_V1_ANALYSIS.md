...
| Huffman compressed U/V    |  9,818,120 bytes | 14,101,564 bytes | +43.63% |
| tiled 量化EB              |  4,027,240 bytes |     67,500 bytes | -98.32% |
| 特殊值(outlier + zero_eb) |  2,188,480 bytes | 14,035,640 bytes |   6.41x |
...
##### 4. 特殊值(outlier + zero_eb)

特殊值(outlier + zero_eb) 同时包含 outlier 和 zero-EB 记录。每条记录都保存一个 `uint32_t index` 和一个 `float value/code`，因此占 `4 + 4 = 8 bytes`（[特殊值文件布局](cpszg_2d_v1.cu#L440-L465)、[写入代码](cpszg_2d_v1.cu#L512-L526)）。

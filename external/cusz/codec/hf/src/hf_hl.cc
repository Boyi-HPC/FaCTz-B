...
int high_level<E>::encode(
    ...
    HF_STREAM stream)
{
  return encode_timed(buf, in, len, out, outlen, header, stream, nullptr);
}

template <typename E>
int high_level<E>::encode_timed(
    Buf<E>* buf, E* in, size_t const len, uint8_t** out, size_t* outlen, phf_header& header,
    HF_STREAM stream, float* kernel_ms)
{
  cudaEvent_t phase12_start = nullptr, phase12_stop = nullptr;
  cudaEvent_t phase4_start = nullptr, phase4_stop = nullptr;
  if (kernel_ms) {
    cudaEventCreate(&phase12_start);
    cudaEventCreate(&phase12_stop);
    cudaEventCreate(&phase4_start);
    cudaEventCreate(&phase4_stop);
    cudaEventRecord(phase12_start, (cudaStream_t)stream);
  }

  phf_module<E>::GPU_coarse_encode_phase1(
      in, len, buf->book_d(), buf->rt_bklen(), buf->numSMs(), buf->scratch_d(), stream);
  phf_module<E>::GPU_coarse_encode_phase2(
      buf->scratch_d(), len, {buf->sublen(), buf->pardeg()}, buf->scratch_d(),
      buf->par_nbit_d(), buf->par_ncell_d(), stream);

  if (kernel_ms) cudaEventRecord(phase12_stop, (cudaStream_t)stream);

  phf_module<E>::GPU_coarse_encode_phase3_sync(
      {buf->sublen(), buf->pardeg()}, buf->par_nbit_d(), buf->par_nbit_h(),
      buf->par_ncell_d(), buf->par_ncell_h(), buf->par_entry_d(), buf->par_entry_h(),
      &header.total_nbit, &header.total_ncell, nullptr, stream);

  if (kernel_ms) cudaEventRecord(phase4_start, (cudaStream_t)stream);
  phf_module<E>::GPU_coarse_encode_phase4(
      buf->scratch_d(), len, buf->par_entry_d(), buf->par_ncell_d(),
      {buf->sublen(), buf->pardeg()}, buf->bitstream_d(), buf->bitstream_max_len(), stream);
  if (kernel_ms) cudaEventRecord(phase4_stop, (cudaStream_t)stream);

  sync_by_stream(stream);

  if (kernel_ms) {
    float phase12_ms = 0.0f, phase4_ms = 0.0f;
    cudaEventElapsedTime(&phase12_ms, phase12_start, phase12_stop);
    cudaEventElapsedTime(&phase4_ms, phase4_start, phase4_stop);
    *kernel_ms = phase12_ms + phase4_ms;
    cudaEventDestroy(phase12_start);
    cudaEventDestroy(phase12_stop);
    cudaEventDestroy(phase4_start);
    cudaEventDestroy(phase4_stop);
  }

...
struct Buf<E>::impl {
  ...
  // constructor
  impl(
      size_t inlen, size_t _bklen, int _pardeg, bool _use_HFR, bool debug,
      bool allocate_host_payload) :
      len(inlen),
      bklen(_bklen),
      ...
      rvbk4_bytes(_rvbk4_bytes(_bklen))
  {
    ...
    // Coarse GPU encode/decode never accesses these full-size host mirrors.
    // Keep them available for callers that use encoded_h() or the HFR path.
    if (allocate_host_payload) h_scratch4 = MAKE_UNIQUE_HOST(H4, len);
    ...
    if (allocate_host_payload || use_HFR)
    ...
    h_encoded = h_scratch4 ? (u1*)h_scratch4.get() : nullptr;
  }
...
PHF_BUF_DEF()::Buf(
    size_t inlen, size_t _bklen, int _pardeg, bool _use_HFR, bool debug) :
    Buf(inlen, _bklen, _pardeg, _use_HFR, debug, true)
{
}
...
PHF_BUF_DEF()::Buf(
    ...
    pimpl(std::make_unique<impl>(
        inlen, _bklen, _pardeg, _use_HFR, debug, allocate_host_payload))
{
}
